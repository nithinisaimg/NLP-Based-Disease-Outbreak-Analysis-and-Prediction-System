...
def predict_category(symptom_text: str) -> str:
    combined = symptom_text.lower()
...
def _try_transformer_summary(text: str) -> Tuple[Optional[str], Dict[str, Any]]:
    ...
    try:
        ...
        if not out or not isinstance(out, list):
            return None, {"transformers": "ok_empty"}
        first = out[0]
        ...
        if not summary:
            return None, {"transformers": "ok_empty"}
        return str(summary).strip(), {"transformers": "ok"}
    except Exception as e:  # noqa: BLE001
        return None, {"transformers": f"failed: {e.__class__.__name__}"}


def analyze_text(symptom_text: str) -> Dict[str, Any]:
    combined = _normalize(symptom_text)

    debug: Dict[str, Any] = {}

    transformer_summary, dbg_sum = _try_transformer_summary(symptom_text) if symptom_text else (None, {})
    debug.update(dbg_sum)
    summary = transformer_summary if transformer_summary else summarize(symptom_text, max_sentences=2)
    ...
    predicted = predict_category(symptom_text=symptom_text)
